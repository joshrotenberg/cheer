defmodule CheerTest do
  ...
  defmodule TestDefaultWithoutStringChars do
    use Cheer.Command

    command "cfgcmd" do
      about("Default value with no String.Chars implementation")
      option(:cfg, type: :string, default: %{mode: :fast}, help: "Config")
    end
  ...
  describe "help text default rendering (issue #68)" do
    test "a default without a String.Chars implementation falls back to inspect/1 instead of crashing" do
      output = capture_io(fn -> Cheer.run(TestDefaultWithoutStringChars, ["--help"]) end)
      assert output =~ "[default: %{mode: :fast}]"
    end
  end
  ...
  defmodule TestHelpValuePosition do
    use Cheer.Command

    command "helpvalue" do
      about("Option that can literally take --help/-h/--version/-V as a value")
      option(:message, type: :string, short: :m, help: "Message")
    end
  ...
  describe "--help/-h/--version/-V only honored in flag position (issue #68)" do
    test "--help is intercepted when it appears as a real flag" do
      output = capture_io(fn -> Cheer.run(TestHelpValuePosition, ["--help"]) end)
      assert output =~ "Option that can literally take"
    end

    test "--help is NOT intercepted when it is the value of a preceding string option" do
      # OptionParser itself refuses a "-"-prefixed token as a value (a
      # separate, existing limitation — see issue #73/#64), so this still
      # ends in a usage error rather than {:ok, _}. The point of this test is
      # that dispatch reaches the *normal* "unknown option(s)" error path
      # instead of short-circuiting straight to the help banner.
      output = capture_io(fn -> Cheer.run(TestHelpValuePosition, ["--message", "--help"]) end)
      assert output =~ "error: unknown option(s): --message, --help"
    end

    test "--version is NOT intercepted when it is the value of a preceding string option" do
      output = capture_io(fn -> Cheer.run(TestHelpValuePosition, ["--message", "--version"]) end)
      assert output =~ "error: unknown option(s): --message, --version"
    end

    test "--help as an inline --flag=value is not intercepted, since it is unambiguously a value" do
      assert {:ok, args} = Cheer.run(TestHelpValuePosition, ["--message=--help"])
      assert args[:message] == "--help"
    end

    test "a real --help flag after a valued option is still intercepted" do
      output =
        capture_io(fn -> Cheer.run(TestHelpValuePosition, ["--message", "hi", "--help"]) end)
  ...
  describe "Cheer.Completion" do
    ...
    test "bash/zsh/fish headers say 'Generated by Cheer', not 'Cheer/Cheer' (issue #68)" do
      for shell <- [:bash, :zsh, :fish] do
        script = Cheer.Completion.generate(TestRoot, shell, prog: "myapp")
  ...
  describe "Cheer.tree" do
    ...
    test "excludes hidden options and arguments (issue #68)" do
      tree = Cheer.tree(CheerTest.TestHiddenOptArg)
      option_names = Enum.map(tree.options, fn {name, _opts} -> name end)
      arg_names = Enum.map(tree.arguments, fn {name, _opts} -> name end)
      assert :visible_opt in option_names
      refute :secret_opt in option_names
      assert :visible_arg in arg_names
      refute :secret_arg in arg_names
    end

    test "includes trailing_var_arg (issue #68)" do
      tree = Cheer.tree(CheerTest.TestTreeTrailingVarArg)
      assert {:rest, _opts} = tree.trailing_var_arg
    end

    test "trailing_var_arg is nil when the command declares none" do
      tree = Cheer.tree(TestRoot)
      assert tree.trailing_var_arg == nil
    end
  end

  defmodule TestHiddenOptArg do
    use Cheer.Command

    command "hiddenoptarg" do
      about("Hidden option/argument in tree")
      argument(:visible_arg, type: :string, required: false)
      argument(:secret_arg, type: :string, required: false, hide: true)
      option(:visible_opt, type: :string, help: "Visible")
      option(:secret_opt, type: :string, hide: true, help: "Secret")
    end
  ...
  defmodule TestTreeTrailingVarArg do
    use Cheer.Command

    command "treetrailer" do
      about("Has a trailing var arg")
      trailing_var_arg(:rest, help: "Everything after")
    end
  ...
  defmodule TestConflictsKebab do
    use Cheer.Command

    command "conflicts-kebab" do
      about("Multi-word option names, to check error messages are kebab-cased")

      option(:dry_run, type: :boolean, conflicts_with: :force_apply, help: "Dry run")
      option(:force_apply, type: :boolean, requires: :confirm_token, help: "Force apply")
      option(:confirm_token, type: :string, help: "Confirmation token")
    end
  ...
  describe "constraint error messages are kebab-cased (issue #68)" do
    test "conflicts_with error uses kebab-case flag names" do
      output =
        capture_io(fn -> Cheer.run(TestConflictsKebab, ["--dry-run", "--force-apply"]) end)

      assert output =~ "error: --dry-run cannot be used with --force-apply"
    end

    test "requires error uses kebab-case flag names" do
      output = capture_io(fn -> Cheer.run(TestConflictsKebab, ["--force-apply"]) end)
      assert output =~ "error: --force-apply requires --confirm-token"
    end
  end
